...
if (!triage.policy?.releaseRule?.includes('No HTTP 301 or 410')) errors.push('R9 must remain staging-only for HTTP lifecycle');
...
const blogFiles = fs.existsSync(blogDir)
  ? fs.readdirSync(blogDir).filter((name) => /\.(md|mdx)$/i.test(name))
  : [];
...
if (defaultHoldCount < 1) errors.push('R9 should leave unreviewed legacy blogs safely default-held rather than implicitly indexable');

for (const protectedSlug of [
  'โน๊ตบุ๊คติดรหัส-windows-ขายได้ไหม',
  'กรณีศึกษา-รับซื้อโน๊ตบุ๊คบอร์ดเสียช็อต-ค่าซ่อมไม่คุ้ม',
  'กรณีศึกษา-รับซื้อโน๊ตบุ๊คองค์กร-20-เครื่อง-ปลด-asset-tag',
  'โน๊ตบุ๊คจอแตกยังมีราคาไหม',
  'โน๊ตบุ๊คเสียขายได้ไหม',
]) {
  if (bySlug.get(protectedSlug)?.action !== 'KEEP_INFORMATIONAL') {
    errors.push(`Protected GSC blog must remain KEEP_INFORMATIONAL: ${protectedSlug}`);
  }
}
...
for (const [slug, target] of [
  ...
  ['ขายโน๊ตบุ๊คที่ไหนดี-ได้ราคาและปลอดภัย', '/ขายโน๊ตบุ๊ค/'],
]) {
  const item = bySlug.get(slug);
  if (!item || item.action !== 'MERGE_TO_OWNER' || item.target !== target) {
    errors.push(`Commercial overlap must MERGE_TO_OWNER: ${slug} => ${target}`);
  }
}

for (const [filePath, tokens] of [
  [blogPagePath, ['rebuild-v2-blog-triage.json', 'resolvedBlogAction', 'lifecycleNoindex', 'MERGE_TO_OWNER', 'lifecycleCanonical', 'noindex={lifecycleNoindex}']],
  ...
  if (!fs.existsSync(filePath)) {
    errors.push(`Missing R9 enforcement file: ${path.relative(root, filePath)}`);
    continue;
  }
...
for (const forbidden of ['return Response.redirect', 'status: 301', 'status: 410']) {
  if (fs.existsSync(blogPagePath) && fs.readFileSync(blogPagePath, 'utf8').includes(forbidden)) {
    errors.push(`R9 must not release HTTP lifecycle in blog route: ${forbidden}`);
  }
}
...
if (keepCount > 40) errors.push(`R9 informational authority set is too broad for this recovery stage: ${keepCount}`);
...
console.log(`- Blog collection files: ${blogFiles.length}`);
console.log(`- KEEP_INFORMATIONAL: ${keepCount}`);
console.log(`- MERGE_TO_OWNER: ${counts.get('MERGE_TO_OWNER') ?? 0}`);
console.log(`- Explicit HOLD_NOINDEX: ${counts.get('HOLD_NOINDEX') ?? 0}`);
console.log(`- RETIRE_CANDIDATE: ${counts.get('RETIRE_CANDIDATE') ?? 0}`);
console.log(`- Default HOLD_NOINDEX: ${defaultHoldCount}`);
console.log('- Blog hub and sitemap expose only reviewed informational owners');
console.log('- Commercial blog overlap is staged away from money owners');
console.log('- No HTTP 301/410 release is claimed in R9');
