...
if (triage.policy?.defaultForUnclassified !== 'HOLD_NOINDEX') {
  errors.push('R6 defaultForUnclassified must be HOLD_NOINDEX');
}

const bySlug = new Map();
const counts = new Map();
for (const item of triage.items ?? []) {
  if (!item.slug || !item.brand || !item.type || !item.action) {
    errors.push(`Incomplete triage row: ${JSON.stringify(item)}`);
    continue;
  }
  if (bySlug.has(item.slug)) errors.push(`Duplicate triage slug: ${item.slug}`);
...
const protectedWinners = new Set([
  'asus-rog-flow',
  'lenovo-legion-pro-7',
  'lenovo-legion-pro',
  'thinkpad-x1-carbon',
...
if (fs.existsSync(inventoryPath)) {
  ...
  for (const slug of inventorySlugs) {
    if (!bySlug.has(slug)) errors.push(`Inventory Series/Model missing triage classification: ${slug}`);
  }
  for (const slug of bySlug.keys()) {
    if (!inventorySlugs.has(slug)) errors.push(`R6 triage slug not found in authority inventory: ${slug}`);
  }
}
...
if (!fs.existsSync(brandLayoutPath)) {
  ...
  for (const token of [
    'rebuild-v2-model-series-triage.json',
    "triageItem?.action === 'HOLD_NOINDEX'",
    "triageItem?.action === 'MERGE'",
    'triageCanonical',
  ]) {
    if (!src.includes(token)) errors.push(`BrandLayout missing R6 enforcement token: ${token}`);
  }
}
