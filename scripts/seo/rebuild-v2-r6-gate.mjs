...
if (fs.existsSync(inventoryPath)) {
  ...
  for (const slug of bySlug.keys()) if (!inventorySlugs.has(slug)) errors.push(`R6 triage slug not found in authority inventory: ${slug}`);
}

if (!fs.existsSync(brandLayoutPath)) errors.push('BrandLayout missing');
else {
  const src = fs.readFileSync(brandLayoutPath, 'utf8');
  for (const token of ['rebuild-v2-model-series-triage.json','modelTriageItem',"lifecycleItem?.action === 'HOLD_NOINDEX'", "lifecycleItem?.action === 'MERGE'",'lifecycleCanonical']) {
...
if (!fs.existsSync(astroConfigPath)) errors.push('astro.config.mjs missing');
  ...
  for (const token of ['rebuild-v2-model-series-triage.json','R6_SITEMAP_INCLUDED','R13_REDIRECT_SOURCES','R13_STAGING_PREFIXES','R5_LEGACY_BRANDS']) {
    if (!src.includes(token)) errors.push(`Sitemap missing release enforcement token: ${token}`);
  }
}
